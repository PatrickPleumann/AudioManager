...
### Kein automatisches Stoppen beim Szenenwechsel
Der AudioManager ist ein `DontDestroyOnLoad`-Singleton und überlebt Szenenwechsel bewusst — für Musik und Ambient, die über Szenen hinweg durchlaufen sollen, ist genau das erwünscht. Die Kehrseite: Ein Szenenwechsel stoppt **keine** Sounds automatisch. Ein szenen-gebundener Loop (z. B. ein Ambient-Sound, der nur zur alten Szene gehört) läuft nach dem Laden weiter, bis er aktiv gestoppt wird — es gibt keinen automatischen „Szene entladen → deren Sounds stoppen"-Mechanismus. **Empfehlung:** Für szenen-gebundene Sounds den beim Abspielen zurückgegebenen `AudioHandle` behalten und vor dem Szenenwechsel selbst `Stop` bzw. `FadeOut` aufrufen.

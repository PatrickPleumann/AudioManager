...
### No automatic stop on scene change
The AudioManager is a `DontDestroyOnLoad` singleton and deliberately survives scene changes — for music and ambient that should carry across scenes, that is exactly what you want. The flip side: a scene change stops **no** sounds automatically. A scene-bound loop (e.g. an ambient sound belonging only to the old scene) keeps playing after the load until it is actively stopped — there is no automatic "scene unloaded → stop its sounds" mechanism. **Recommendation:** For scene-bound sounds, keep the `AudioHandle` returned at play time and call `Stop` or `FadeOut` yourself before the scene change.

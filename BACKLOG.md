...
- [ ] **Mixer/Bus + Ducking** — Laufzeit-Volume (Settings-Menü) + per-Kategorie-Ducking (z. B. Dialog senkt Musik), optional an Unity-AudioMixer-Groups koppelbar. Baut auf dem bestehenden `AudioCategory`/`VolumeDictionary`-System auf. **Höchster Verkaufshebel** (fast jedes Spiel braucht Volume-Menü + Ducking).
  ...
  - **Test-getriebene Schritt-Kette (jeder Schritt rot-zuerst, System bleibt lauffähig):**
    - **Stand 2026-07-02:** Schritte **1 + 2 + 3 erledigt.** Schritt 1 — `VolumeResolver.Resolve(basis, fade, duck)` (committet `deab831`). Schritt 2 — `DuckEnvelope.Step(...)` in `Services/DuckEnvelope.cs` (Namespace `AudioFramework.Services.Mixing`) + 8 EditMode-Tests; committet `8466ac4` (die frühere „noch nicht committet"-Notiz war veraltet). Schritt 3 — `DuckTargetPolicy.ResolveDuck(target, activeCategories, pairs)` + pure Input-Struct `DuckPair` (Feld `DuckedVolume`, multiplikative Faktor-Semantik, **keine Inversion**) in `Services/DuckTargetPolicy.cs` + `Services/DuckPair.cs` + 8 EditMode-Tests in `DuckTargetPolicyTests.cs`. Voller Loop **gate-für-gate** durchlaufen (rot → grün → Mutation [Selbst-Duck-Guard entfernt → nur `SelfDuckSkipped` rot, wie vorhergesagt] → grün), jeder Übergang von Patrick bestätigt. **Noch nicht committet** (Patrick committet selbst). **Nächster Schritt: 4 — Config-Daten `DuckRule`.**
    ...
    4. Config-Daten: `DuckRule { trigger, ziele: [{ kategorie, tiefe }] }` + globale Attack/Release; **plus** reservierter (leerer) `Kategorie → AudioMixerGroup`-Seam für Stufe 2. Leichte `OnValidate`-Guards. **UI-Entscheidung (2026-07-02): Faktor-Semantik bleibt, keine Inversion in der puren Schicht — die Verständlichkeit kommt allein über den Inspector-Tooltip, wörtlich „duckt die Ziel-Kategorien AUF X % ihrer Lautstärke". Der User tippt direkt den Faktor (0.9 = auf 90 %), der Text erklärt ihn.**
    5. `AudioCategory` auf `AudioObject` (unbedingt bei jedem Dispatch geschrieben).
    6. Runtime-Glue `AudioDuckService` (im Manager-Tick): aktive Kategorien ableiten → Ziele (3) → gliden (2) → `source.volume` auflösen (1). PlayMode-Smoke. Hier fällt der **Live-Settings-Slider** gratis ab.

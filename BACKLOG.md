...
- [ ] **Mixer/Bus + Ducking** — Laufzeit-Volume (Settings-Menü) + per-Kategorie-Ducking (z. B. Dialog senkt Musik), optional an Unity-AudioMixer-Groups koppelbar. Baut auf dem bestehenden `AudioCategory`/`VolumeDictionary`-System auf. **Höchster Verkaufshebel** (fast jedes Spiel braucht Volume-Menü + Ducking).
  ...
  - **Test-getriebene Schritt-Kette (jeder Schritt rot-zuerst, System bleibt lauffähig):**
    - **Stand 2026-06-28:** Schritt **1 erledigt** — `VolumeResolver.Resolve(basis, fade, duck)` in `Assets/Scripts/AudioFramework/Services/VolumeResolver.cs` (Namespace `AudioFramework.Services.Mixing`) + 8 EditMode-Tests in `VolumeResolverTests.cs`. Voller Loop durchlaufen: rot-zuerst (Stub) → grün → Mutation-Check (zwei Lehren ehrlich in `TEST_VERIFICATION.md` #12 festgehalten) → korrekter Zustand wiederhergestellt. **Noch nicht committet** (Patrick committet selbst). **Nächster Schritt: 2 — `DuckEnvelope.Step`** (Glide, Zwilling von `OcclusionSmoothing.Step`).

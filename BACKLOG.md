...
- [ ] **Mixer/Bus + Ducking** — Laufzeit-Volume (Settings-Menü) + per-Kategorie-Ducking (z. B. Dialog senkt Musik), optional an Unity-AudioMixer-Groups koppelbar. Baut auf dem bestehenden `AudioCategory`/`VolumeDictionary`-System auf. **Höchster Verkaufshebel** (fast jedes Spiel braucht Volume-Menü + Ducking).
  ...
  - **Test-getriebene Schritt-Kette (jeder Schritt rot-zuerst, System bleibt lauffähig):**
    - **Stand 2026-06-28:** Schritte **1 + 2 erledigt.** Schritt 1 — `VolumeResolver.Resolve(basis, fade, duck)` (committet). Schritt 2 — `DuckEnvelope.Step(current, target, deltaTime, attackRate, releaseRate)` in `Assets/Scripts/AudioFramework/Services/DuckEnvelope.cs` (Namespace `AudioFramework.Services.Mixing`) + 8 EditMode-Tests in `DuckEnvelopeTests.cs`. Zwilling von `OcclusionSmoothing.Step`, aber asymmetrische Rate: Richtung wählt Attack (Faktor fällt = tiefer ducken) vs. Release (Faktor steigt = zurück). Voller Loop **gate-für-gate** durchlaufen (rot → grün → Mutation [Attack/Release vertauscht → 4 von 8 rot, wie vorhergesagt] → grün), jeder Übergang von Patrick bestätigt. **Noch nicht committet** (Patrick committet selbst). **Nächster Schritt: 3 — `DuckTargetPolicy`.**
    ...
    3. Pure `DuckTargetPolicy` (aktive Kategorien + Paar-Tiefen → `duck[kategorie]`; `min`-Stacking; Kategorie duckt sich nie selbst).
    4. Config-Daten: `DuckRule { trigger, ziele: [{ kategorie, tiefe }] }` + globale Attack/Release; **plus** reservierter (leerer) `Kategorie → AudioMixerGroup`-Seam für Stufe 2. Leichte `OnValidate`-Guards.

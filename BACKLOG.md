...
- [ ] **Mixer/Bus + Ducking** — Laufzeit-Volume (Settings-Menü) + per-Kategorie-Ducking (z. B. Dialog senkt Musik), optional an Unity-AudioMixer-Groups koppelbar. Baut auf dem bestehenden `AudioCategory`/`VolumeDictionary`-System auf. **Höchster Verkaufshebel** (fast jedes Spiel braucht Volume-Menü + Ducking).
  ...
  - **Noch offene Fragen (vor Implementierung klären):**
    - **(offen) Kaskaden-Semantik:** Triggert eine Kategorie, die selbst gerade geduckt wird, weiterhin ihre eigenen Regeln? Claudes **starke Empfehlung: ja — geduckter Sound = voller Trigger, KEINE Kaskade/Fixpunkt-Iteration** (sonst Oszillation + Reihenfolge-Hölle). Per-Paar-Tiefen machen Ketten (A duckt B, B duckt C) wahrscheinlicher → bewusst entscheiden. *Von Patrick noch nicht bestätigt.*
    - **(offen) Stufe-2-Scope/Timing:** Reverb-Zonen, Sends, Snapshots, `outputAudioMixerGroup`-Routing — wann (falls überhaupt vor 1.0)? Reverb-Zonen brauchen nur `reverbZoneMix` als Control-Surface-Feld (billig); Sends/Snapshots sind reines Unity-Wiring + dünne testbare Trigger-API. Entscheiden, sobald B steht.
